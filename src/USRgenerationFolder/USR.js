...
const USR = () => {
  ...
  useEffect(() => {
    ...
    // const result = require("http://localhost:9999/USR2");
    setSelectedData(result[index]);
    setLoading(false);
    finalJson=result[index];
  }, [index, selectedData]);
  ...
  return (
    ...
    <label for="status">Status:</label><input type="text" id="status" value={reviewStatus === "in-edit" ? "In Edit" : "In Review"} disabled/>
    </div>
